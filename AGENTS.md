...
## 岗位 08 子方向（2026-09-19 决定）

岗位 08（字节 AI 全栈-客服平台）此前未分配到任何项目，现决定**挂在本仓库下作为子方向**，不新增独立项目。

客服平台与本项目的电商多 Agent 场景有天然重合（工单/会话处理、工具调用、人工接管），可复用同一套 trace 与评测基建。**但 08 的 JD 含全栈/前端要求，本项目不做前端**，这部分不声称覆盖——具体差距在技术方案里如实列出。
